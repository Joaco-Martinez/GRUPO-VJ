  ...
  return (
    <button
      ...
      className={[
        ...
        checked ? 'border-emerald-500/20 bg-emerald-500/10 text-emerald-300' : 'border-white/10 bg-white/5 text-slate-300 hover:bg-white/10',
      ].join(' ')}
    >
  ...
  return (
    <button
      ...
      className={[
        ...
        checked ? 'border-indigo-400/30 bg-indigo-500/15 text-indigo-200 ring-2 ring-indigo-500/15' : 'border-white/10 bg-white/5 text-slate-300 hover:bg-white/10',
      ].join(' ')}
    >
      {label}
    </button>
      ...
      <div className="mt-6 overflow-hidden rounded-3xl border border-white/10">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[650px] text-left text-sm">
            ...
            <tbody className="divide-y divide-white/10 bg-[#0d1224]">
              {points.length > 0 ? (
                points.map((point) => (
                  <tr key={point.id} className="transition hover:bg-white/5">
                    <td className="px-5 py-4 font-mono text-base font-black text-white">{String(point.number).padStart(4, '0')}</td>
                    <td className="px-5 py-4 font-medium text-slate-300">{point.description || '—'}</td>
                    <td className="px-5 py-4">
                      <div className="flex flex-wrap gap-2">
                        ...
                        <span className={['rounded-full px-3 py-1 text-xs font-bold', point.enabled ? 'border border-emerald-500/20 bg-emerald-500/10 text-emerald-200' : 'border border-white/10 bg-white/5 text-slate-400'].join(' ')}>
                          {point.enabled ? 'Activo' : 'Inactivo'}
                        </span>
                      </div>
                    </td>
                    <td className="px-5 py-4 font-mono text-xs font-semibold text-slate-400">{point.enabledCbteTypes?.join(', ') || '—'}</td>
                    <td className="px-5 py-4 text-right">
                      <button type="button" onClick={() => onDelete(point.id)} className="inline-flex h-10 w-10 items-center justify-center rounded-2xl text-slate-500 transition hover:bg-rose-500/10 hover:text-rose-300" title="Eliminar">
                        <Trash2 size={17} />
                      </button>
                    </td>
                  </tr>
                ...
                <tr>
                  <td colSpan={5} className="px-5 py-12 text-center">
                    <div className="mx-auto flex max-w-sm flex-col items-center">
                      <div className="mb-3 flex h-14 w-14 items-center justify-center rounded-3xl border border-white/10 bg-white/5 text-slate-400">
                        <ServerCog size={24} />
                      </div>
                      <p className="font-black text-white">Sin puntos de venta</p>
                      <p className="mt-1 text-sm leading-relaxed text-slate-400">Agregá el punto de venta Web Service que configuraste en ARCA.</p>
                    </div>
                  </td>
                </tr>
              )}
